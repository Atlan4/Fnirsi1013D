...
void scope_do_auto_setup(void)
{
 ...
 // uint32    center; 
  ...
  for(voltperdiv=0;voltperdiv<6 && flags!=0;voltperdiv++)
  {
    ...
    max1 = (settings->max * multiply ) >> VOLTAGE_SHIFTER;//3289650
    center1x = (settings->center * multiply ) >> VOLTAGE_SHIFTER;//3289650
    min1 = (settings->min * multiply ) >> VOLTAGE_SHIFTER;//3289650
    p2p1 = (settings->peakpeak * multiply ) >> VOLTAGE_SHIFTER;//3289650
  ...
  if(scopesettings.channel2.enable)
  {
    ...
    {
      //Calculate a new trace offset
        scopesettings.channel2.traceposition += (center2 - center1x);
