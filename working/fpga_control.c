...
void fpga_set_trigger_level(void)
{
  ...
  else
  {
    ...
    dcoffset = (scopesettings.channel2.dcoffset*100)/230;//195
...
//----------------------------------------------------------------------------------------------------------------------------------
//short time base
void fpga_set_time_base(uint32 timebase) //len pre rezim auto, urcije max cas po ktory sa odoberaju vzorky
{
...
void fpga_read_sample_data(PCHANNELSETTINGS settings, uint32 triggerpoint)
{
  ...
  threshold = ((settings->max - settings->min) / 10) + 2;//+2
...
//-*****************************************************************************    
 ...
 //   sample+=settings->dcoffset/4;
    sample+=((settings->dcoffset*10)/39); //39 korekcia nameranej hodnoty 0 vs Vmin
