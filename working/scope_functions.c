...
void scope_adjust_timebase(void)
{
  ...
  if(!scopesettings.waveviewmode) {min = 0;  max = ((sizeof(time_div_texts) / sizeof(int8 *)) - 1); }
    ...
    if((scopesettings.runstate))//ok //ked bezi urob
...
{   
  ...
  /*
          //Set the selected sample rate
        fpga_set_sample_rate(samplerate_for_autosetup[3]);

        //Set the matching time base
        fpga_set_time_base(sample_rate_time_per_div[samplerate_for_autosetup[3]]);
        
     */   
        settings->ADoverload = 0;
        /*
         while((settings->samplevoltperdiv < 3)&&(settings->ADoverload == 0)) 
        {
...
//-------------------------------------------------------------------------------------------------------------------------  
  ...
  //-********************************************************************************************
    ...
    p2pCH1 = (settings->peakpeak * multiply ) >> VOLTAGE_SHIFTER;//3289650
    
      //Check if signal in good range 
      if((max >= 301) || (min < 100) || (p2pCH1 > minvpplevel))
    ...
    center = ((settings->center) * multiply ) >> VOLTAGE_SHIFTER;//3289650
    min = (settings->min * multiply ) >> VOLTAGE_SHIFTER;//3289650
    p2pCH2 = (settings->peakpeak * multiply ) >> VOLTAGE_SHIFTER;//3289650
    
      //Check if signal in good range 
      if((max >= 301) || (min < 100) || (p2pCH2 > minvpplevel)) 
      {
        //When signal in range stop scanning the channel
         if (settings->ADoverload) 
            {
  ...
  if(scopesettings.channel1.enable)
  {
   ...
   if(!settings->coupling)
   {
    ...
    if(center < center1)
    {
      ...
      settings->traceposition += ((center1 - center)*120)/100;
  ...
  if(scopesettings.channel2.enable)
  {
   ...
   if(!settings->coupling)
   {
    ...
    {
        //Calculate a new trace offset
        settings->traceposition += ((center2 - center)*120)/100;
...
int32 scope_get_y_sample(PCHANNELSETTINGS settings, int32 index)
{
  ...
  //sample = settings->traceposition + sample + settings->dcoffset/2;
  sample = settings->traceposition + sample + (settings->dcoffset*100)/220;//250
  //sample = settings->traceposition + sample + (settings->dcoffset*100)/195;//250
  //sample = 202 + sample;
...
void scope_prepare_setup_for_file(void)
{
  ...
  //Leave space for file version and checksum data  18
  ...
  //Leave some space for channel 1 settings changes 18
  ...
  //Leave some space for channel 2 settings changes 10
  ...
  //Leave some space for trigger information changes    8
  ...
  //Leave some space for other scope settings changes   6
...
void scope_reset_config_data(void)
{
  ...
  //Set the related acquisition speed which is 200KSa/s
  scopesettings.samplerate = 9;//9 16 5+11
  ...
  //-------------------------------------------------------------------------------------------------
  //Default settings for input calibration value (if SETTING_SECTOR_VERSION_HIGH is set to 0x0100 )
  if (SETTING_SECTOR_VERSION_HIGH == 0x0100)  
    ...
    {
    //Only load default settings SCOPE
...
void scope_save_config_data(void)
{
  ...
  //Point to the channel 1 settings                 9
  ...
  *ptr++ = scopesettings.channel1.dcoffset;
  ...
  //Point to the channel 2 settings                 9
  ...
  *ptr++ = scopesettings.channel2.dcoffset;
  ...
  //Point to the trigger settings                   8
  ...
  *ptr++ = scopesettings.triggerverticalposition;
  ...
  //Point to the other settings                     10
  ...
  *ptr++ = onoffRTC;
  ...
  //Point to the cursor settings                    6
