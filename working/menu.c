  ...
  while(1)
  {
    ...
    if(havetouch)
    {
      ...
      //if((xtouch >= 550) && (xtouch <= 630) && (ytouch >= 160) && (ytouch <= 240))// x596 y203    
        ...
        //Add the ok button
        scope_display_ok_button(460, 400, 0);//550 360
      ...
      //************************
      //Check if touch within the text field
      if(( diag_screen == 2) && (xtouch >= 460) && (xtouch <= 490) && (ytouch >= 415) && (ytouch <= 440))//360 390 415 440  //560 590 415 440
        {
        //Highlight the button
        scope_display_ok_button(460, 400, 1);//550 360
...
//const uint16    k=700;
  ...
  //Display the temperature with font 3 and the orange & white color
  display_set_font(&font_3);
  display_set_fg_color(DARKGREEN_COLOR);
  display_text(b, j+135, "Temperature");
  display_set_fg_color(WHITE_COLOR);
  temp = readtemperature();
  
  //Display sign (-)
  if (!(temp & 0x80)) b1=b; else {b1=b+10; display_text(b, j+150, "-");}
  
  display_decimal(b1, j+150, temp & 0x7F);
  display_text(b1+20, j+150, "C"); 
  ...
  display_text(360, 405, "RESET");                      //670 260
  ...
  display_text(b, i-20, "Channel 1");
  
  //Display the text with the channel2 color
  display_set_fg_color(CHANNEL2_COLOR);
  display_text(b, j-20, "Channel 2");
  
  
  display_set_fg_color(DARKGREEN_COLOR);
  display_text(a, i, "ADc comp"); 
  display_set_fg_color(RED_COLOR);
  display_text(a, i+45, "DC offset");
    
  //Display the text with the white color
  display_set_fg_color(WHITE_COLOR);
  display_decimal(a, i+15, scopesettings.channel1.adc1compensation);
  display_decimal(a, i+30, scopesettings.channel1.adc2compensation);
  display_decimal(a, i+60, scopesettings.channel1.dcoffset);
  //display_decimal(a, i+75, scopesettings.channel1.adc2compensation);  
  
  display_set_fg_color(DARKGREEN_COLOR);  
  display_text(a, j, "ADc comp"); 
  display_set_fg_color(RED_COLOR); 
  display_text(a, j+45, "DC offset");  
  
  //Display the text with the white color
  display_set_fg_color(WHITE_COLOR);    
  display_decimal(a, j+15, scopesettings.channel2.adc1compensation);
  display_decimal(a, j+30, scopesettings.channel2.adc2compensation);
  display_decimal(a, j+60, scopesettings.channel2.dcoffset);
  //display_decimal(a, j+75, scopesettings.channel1.adc2compensation);  
...
void scope_acqusition_settings(int mode)
{
  ...
  if (scopesettings.long_mode)
  {
      ...
      display_text(ACQ_BUTTON_XPOS + 2, ACQ_BUTTON_YPOS + 20, "Long"); 
...
void scope_move_speed(int mode)
{
  ...
  {
    //Active so pale yellow background
    display_set_fg_color(0x00FFFF80); //0x00FFFF80
  }
...
void scope_open_channel_menu(PCHANNELSETTINGS settings)
{
  ...
  //Three black lines between the settings
  display_set_fg_color(BLACK_COLOR);
  display_draw_horz_line(CH_MENU_YPOS +  60, xstart, xend); //62
  display_draw_horz_line(CH_MENU_YPOS + 120, xstart, xend);
  display_draw_horz_line(CH_MENU_YPOS + 180, xstart, xend); //186?
  
  //Set channel color
  //display_set_fg_color(settings->color);
  display_draw_horz_line(CH_MENU_YPOS + 240, xstart, xend); //248
  
  //One black lines between the sensitivity
  display_set_fg_color(BLACK_COLOR);
  display_draw_vert_line(settings->menuxpos + 230,CH_MENU_YPOS+4, CH_MENU_YPOS+239);//10 248
  
  //Set channel color
  display_set_fg_color(settings->color);
  //One canal_color lines the settings and sensitivity
  display_draw_vert_line(settings->menuxpos + 171,CH_MENU_YPOS+4, CH_MENU_YPOS+239);//248
  ...
  //Display the texts
  display_text(settings->menuxpos + 15, CH_MENU_YPOS +  12, "Open");//10
  display_text(settings->menuxpos + 22, CH_MENU_YPOS +  31, "CH");//29
  display_text(settings->menuxpos + 15, CH_MENU_YPOS +  72, "Open");
  display_text(settings->menuxpos + 19, CH_MENU_YPOS +  91, "FFT");
  display_text(settings->menuxpos + 15, CH_MENU_YPOS + 132, "Coup");//136
  display_text(settings->menuxpos + 18, CH_MENU_YPOS + 151, "ling");//154
  display_text(settings->menuxpos + 15, CH_MENU_YPOS + 192, "V / A");//201
  display_text(settings->menuxpos + 12, CH_MENU_YPOS + 211, "mode");//219
  //display_text(settings->menuxpos + 15, CH_MENU_YPOS + 201, "probe");
  //display_text(settings->menuxpos + 15, CH_MENU_YPOS + 219, "mode");
  display_text(settings->menuxpos + 15, CH_MENU_YPOS + 252, "Probe");//266
  display_text(settings->menuxpos + 15, CH_MENU_YPOS + 271, "mode");//284
...
void scope_channel_enable_select(PCHANNELSETTINGS settings)
{
  ...
  if(settings->enable == 0)
  {
    ...
    display_fill_rect(settings->menuxpos + 78, CH_MENU_YPOS + 19, 32, 22);//17
  }
  else
  {
    //Enabled so dark grey box behind off
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 19, 32, 22);//17
  }
  ...
  if(settings->enable == 0)
  {
    ...
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 19, 32, 22);//16
  }
  else
  {
    //Enabled so channel color box behind on
    display_fill_rect(settings->menuxpos + 78, CH_MENU_YPOS + 19, 32, 22);//16
  }
  ...
  //Display the on text
  display_text(settings->menuxpos + 84, CH_MENU_YPOS + 22, "ON");//20
  ...
  display_text(settings->menuxpos + 133, CH_MENU_YPOS + 22, "OFF");//20
}
...
void scope_channel_fft_show(PCHANNELSETTINGS settings)
{
  ...
  //Display the on text
  display_text(settings->menuxpos + 84, CH_MENU_YPOS + 82, "ON");//82
  ...
  //Display the off text
  display_text(settings->menuxpos + 133, CH_MENU_YPOS + 82, "OFF");//82
}
...
void scope_channel_coupling_select(PCHANNELSETTINGS settings)
{
  ...
  if(settings->coupling == 0)
  {
    ...
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 139, 32, 22);//142
  }
  else
  {
    //AC so dark grey box behind dc text
    display_fill_rect(settings->menuxpos + 78, CH_MENU_YPOS + 139, 32, 22);
  }
  ...
  else
  {
    ...
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 139, 32, 22);
  }
  ...
  //Display the DC text
  display_text(settings->menuxpos + 85, CH_MENU_YPOS + 142, "DC");//145
  ...
  //Display the off text
  display_text(settings->menuxpos + 137, CH_MENU_YPOS + 142, "AC");//145
}
//----------------------------------------------------------------------------------------------------------------------------------
...
void scope_channel_VA_select(PCHANNELSETTINGS settings)
{
  ...
  if(settings->V_A == 0)
  {
    ...
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 199, 32, 22);//142
  }
  else
  {
    //A so dark grey box behind V text
    display_fill_rect(settings->menuxpos + 78, CH_MENU_YPOS + 199, 32, 22);
  }
  ...
  {
    //AC so channel color box behind ac text
    display_fill_rect(settings->menuxpos + 130, CH_MENU_YPOS + 199, 32, 22);
  }
  ...
  //Display the DC text
  display_text(settings->menuxpos + 90, CH_MENU_YPOS + 202, "V");//85 202
  ...
  //Display the off text
  display_text(settings->menuxpos + 142, CH_MENU_YPOS + 202, "A");//137 202
}
...
void scope_channel_probe_magnification_select(PCHANNELSETTINGS settings)
{
  ...
  display_fill_rect(settings->menuxpos + 67,  CH_MENU_YPOS + 250, 213, 40);//72
  ...
  switch(settings->magnification)
  {
    case 0:
      ...
      display_fill_rect(settings->menuxpos + 67, CH_MENU_YPOS + 250, 25, 40);//72
      break;

    case 1:
      //Highlight times 1 magnification    //1
      display_fill_rect(settings->menuxpos + 97, CH_MENU_YPOS + 250, 25, 40);//102
      break;
      
    case 2:
      //Highlight times 10 magnification     //0 77
      display_fill_rect(settings->menuxpos + 127, CH_MENU_YPOS + 250, 25, 40);//132
      break;

    case 3:
      //Highlight times 10 magnification    //1 107
      display_fill_rect(settings->menuxpos + 157, CH_MENU_YPOS + 250, 25, 40);//162
      break;  
   
    case 4:
      //Highlight times 100 magnification   //default   137
      display_fill_rect(settings->menuxpos + 187, CH_MENU_YPOS + 250, 25, 40);//192
      break;
      
    case 5:
      //Highlight times 500 magnification   
      display_fill_rect(settings->menuxpos + 217, CH_MENU_YPOS + 250, 25, 40);//222
      break;
      
    case 6:
      //Highlight times 1000 magnification  
      display_fill_rect(settings->menuxpos + 249, CH_MENU_YPOS + 250, 31, 40);//252
      break;
  ...
  {
    //Yes so black 0.2X text
    display_set_fg_color(BLACK_COLOR);
  }
  else
  {
    //No so white or grey 0.2 text
    if(scopesettings.waveviewmode)
    {  
      //Grey auto text when in waveform view mode
      display_set_fg_color(0x00606060);
  ...
  //Display the 0.2X text
  display_text(settings->menuxpos + 71, CH_MENU_YPOS + 252, "0.5"); //+30 76
  display_text(settings->menuxpos + 75, CH_MENU_YPOS + 271, "X");   //+34 80
  //--------------------------------------------------------------------
 //Check if magnification is 1x
  ...
  display_text(settings->menuxpos + 106, CH_MENU_YPOS + 252, "1");//+27
  display_text(settings->menuxpos + 105, CH_MENU_YPOS + 271, "X");// +31
  ...
  //Display the 1X text
  display_text(settings->menuxpos + 132, CH_MENU_YPOS + 252, "10");// +27 na 10x
  display_text(settings->menuxpos + 135, CH_MENU_YPOS + 271, "X");// +31 na x10
  //--------------------------------------------------------------------
  ...
  display_text(settings->menuxpos + 163, CH_MENU_YPOS + 252, "20");// +30 na x100
  display_text(settings->menuxpos + 165, CH_MENU_YPOS + 271, "X");//  +34 na x100
  //--------------------------------------------------------------------
  //Check if magnification is 50x
  ...
  //Display the 10X text
  display_text(settings->menuxpos + 193, CH_MENU_YPOS + 252, "50");// +30 na x100
  display_text(settings->menuxpos + 195, CH_MENU_YPOS + 271, "X");//  +34 na x100
  //--------------------------------------------------------------------
  ...
  display_text(settings->menuxpos + 219, CH_MENU_YPOS + 252, "100");//194
  display_text(settings->menuxpos + 225, CH_MENU_YPOS + 271, "X");//200
  //--------------------------------------------------------------------
  /*
  ...
  display_text(settings->menuxpos + 250, CH_MENU_YPOS + 252, "1000");//257 1k
  display_text(settings->menuxpos + 260, CH_MENU_YPOS + 271, "X");
//******************************************************************************
}
...
{  
  ...
  display_fill_rect(settings->menuxpos + 237, CH_MENU_YPOS + 139, 46, 22);//145
  ...
  display_fill_rect(settings->menuxpos + 237, CH_MENU_YPOS + 79, 46, 22);//82
  ...
  display_fill_rect(settings->menuxpos + 237, CH_MENU_YPOS + 19, 46, 22);//20
  ...
  display_fill_rect(settings->menuxpos + 178, CH_MENU_YPOS + 199, 46, 22);//209
  ...
  display_fill_rect(settings->menuxpos + 178, CH_MENU_YPOS + 139, 46, 22);//145
  ...
  display_fill_rect(settings->menuxpos + 178, CH_MENU_YPOS + 79, 46, 22);//82
  ...
  display_fill_rect(settings->menuxpos + 178, CH_MENU_YPOS + 19, 46, 22);//20

  //Select the font for the texts
  display_set_font(&font_3);

  
  //display_text(settings->menuxpos + 185, CH_MENU_YPOS + 20,  volt_div_texts[settings->magnification][settings->displayvoltperdiv]);
  //prida ttabulku offsetov a farieb pozriet ako robi ponuku pre ACQ menu
  ...
  display_text(settings->menuxpos + 185, CH_MENU_YPOS + 22,  "50mV");//20
  if (settings->displayvoltperdiv==5)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 180, CH_MENU_YPOS + 82,  "100mV");//82
  if (settings->displayvoltperdiv==4)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 180, CH_MENU_YPOS + 142, "200mV");//145
  if (settings->displayvoltperdiv==3)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 180, CH_MENU_YPOS + 202, "500mV");//209
  if (settings->displayvoltperdiv==2)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 253, CH_MENU_YPOS + 22,  "1V");//20
  if (settings->displayvoltperdiv==1)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 248, CH_MENU_YPOS + 82,  "2.5V");//82
  if (settings->displayvoltperdiv==0)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 253, CH_MENU_YPOS + 142, "5V"); break;//145
  
  case 2:
  if (settings->displayvoltperdiv==6)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);    
  display_text(settings->menuxpos + 180, CH_MENU_YPOS + 22,  "500mV");
  if (settings->displayvoltperdiv==5)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 195, CH_MENU_YPOS + 82,  "1V");
  if (settings->displayvoltperdiv==4)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 195, CH_MENU_YPOS + 142, "2V");
  if (settings->displayvoltperdiv==3)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 195, CH_MENU_YPOS + 202, "5V");
  if (settings->displayvoltperdiv==2)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 249, CH_MENU_YPOS + 22,  "10V");
  if (settings->displayvoltperdiv==1)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 249, CH_MENU_YPOS + 82,  "25V");
  if (settings->displayvoltperdiv==0)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 249, CH_MENU_YPOS + 142, "50V"); break;
  
  case 3:
  if (settings->displayvoltperdiv==6)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);    
  display_text(settings->menuxpos + 195, CH_MENU_YPOS + 22,  "5V");
  if (settings->displayvoltperdiv==5)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 190, CH_MENU_YPOS + 82,  "10V");
  if (settings->displayvoltperdiv==4)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 190, CH_MENU_YPOS + 142, "20V");
  if (settings->displayvoltperdiv==3)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 190, CH_MENU_YPOS + 202, "50V");
  if (settings->displayvoltperdiv==2)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 245, CH_MENU_YPOS + 22,  "100V");
  if (settings->displayvoltperdiv==1)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 245, CH_MENU_YPOS + 82,  "250V");
  if (settings->displayvoltperdiv==0)display_set_fg_color(BLACK_COLOR); else display_set_fg_color(WHITE_COLOR);
  display_text(settings->menuxpos + 245, CH_MENU_YPOS + 142, "500V"); break;
  ...
  if(triger50)
    ...
    //Set channel color for the box behind the selected text
    display_set_fg_color(settings->color);
    display_fill_rect(settings->menuxpos + 237, CH_MENU_YPOS + 199, 45, 22);//209
    //Texts in black
    display_set_fg_color(BLACK_COLOR);
    display_text(settings->menuxpos + 250, CH_MENU_YPOS + 202, "50%");//209
    }
    else      
    {
    //Texts in white
    display_set_fg_color(WHITE_COLOR);
    display_text(settings->menuxpos + 250, CH_MENU_YPOS + 202, "50%");
    }
}
...
void scope_acquisition_speed_select(void)
{
  ...
  for(i=0;i<((sizeof(acquisition_speed_texts) / sizeof(int8 *))-9);i++)//11
  {
  ...
  if(scopesettings.timeperdiv>8) display_fill_rect(ACQ_MENU_XPOS + x, ACQ_MENU_YPOS + y, 68, 20);
  ...
  for(i=0;i<((sizeof(acquisition_speed_texts) / sizeof(int8 *))-11);i++)
  {
    if(i != scopesettings.samplerate)
    {
      ...
      if((scopesettings.runstate)&&(!scopesettings.long_mode))
      {
  ...
  x+= 67;
  y += 40;
  display_text(ACQ_BUTTON_XPOS + x, ACQ_BUTTON_YPOS + y, "Average");
  ...
  { //Set orange color for the text
    display_set_fg_color(ORANGE_COLOR);
    display_text(ACQ_BUTTON_XPOS + x, ACQ_BUTTON_YPOS + y, "ACQ Long");
  ...
  else 
  {
    ...
    display_set_fg_color(GREEN_COLOR);
    display_text(ACQ_BUTTON_XPOS + x, ACQ_BUTTON_YPOS + y, "ACQ Short");
  }
...
void scope_trigger_edge_select(void)
{
  ...
  display_text(672, 127, "Falling");//674
}
