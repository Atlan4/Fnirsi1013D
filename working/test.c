  ...
  void scope_preset_values(void)
{
    ...
    ////Scope mode clear the sample memory
   /*f(!scopesettings.waveviewmode) 
    {
...
void scope_get_long_timebase_data(void)
{
  ...
  uint32    curticks;
      ...
      if((scopesettings.runstate)&&(!triggerlong)&&(scopesettings.triggermode))//runstate ok
        {scope_check_long_trigger();}
  
  //Check if run or stop mode - 1-Run mode
  if((scopesettings.runstate)&&(triggerlong))//ok   
  {
