...
{ 
  ...
  fpga_write_cmd(0x1B);
  fpga_write_short(scopesettings.settriggerpoint/2);
    
  //Write the command to set the total_samples to the FPGA
  fpga_write_cmd(0x1C);
  fpga_write_short(scopesettings.totalsamples/2);
...
uint16 fpga_prepare_for_transfer(void)
{
  ...
  data = (fpga_read_short() & 0x1FFF);  //change 11.12.2023
...
void fpga_read_adc_data(PCHANNELSETTINGS settings)
{
  ...
  count = scopesettings.totalsamples/2;
