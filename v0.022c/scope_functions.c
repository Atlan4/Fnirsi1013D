...
void scope_input_calibration(void)
{
    ...
    switch (calibrationopen)
      ...
      //scopesettings.channel1.samplevoltperdiv = vic; 
      scopesettings.channel2.displayvoltperdiv = vic;    
      //scopesettings.channel2.samplevoltperdiv = vic;
    ...
    //Set the new voltperdiv in the FPGA
    //fpga_set_channel_voltperdiv(&scopesettings.channel1);
    //fpga_set_channel_voltperdiv(&scopesettings.channel2);
...
void scope_do_50_percent_trigger_setup(void)
{
  ...
  int16  ch1_min=255-scopesettings.channel1.min;
  int16  ch1_max=scopesettings.channel1.max;
  int16  ch2_min=255-scopesettings.channel2.min;
  int16  ch2_max=scopesettings.channel2.max;
  ...
  if(scopesettings.triggerchannel == 0)
  {
    ...
    //scopesettings.channel1.triggerverticalposition = scopesettings.channel1.traceposition;
    // scopesettings.channel1.triggerverticalposition = (ch1_max+ch1_min)/2 - ch1_min +scopesettings.channel1.traceposition;
      
    scopesettings.channel1.triggerverticalposition = scopesettings.channel1.center-128+scopesettings.channel1.traceposition + (ch1_max-ch1_min)/2;
    //scopesettings.channel1.triggerverticalposition = scopesettings.channel1.center-128+scopesettings.channel1.traceposition;
    
    //scopesettings.channel1.triggerverticalposition = scopesettings.channel1.center-scopesettings.channel1.min+scopesettings.channel1.traceposition;
    //scopesettings.channel1.triggerverticalposition = scopesettings.channel1.max-scopesettings.channel1.center+scopesettings.channel1.traceposition;
    //scopesettings.channel1.triggerverticalposition = (scopesettings.channel1.center-128+(scopesettings.channel1.max-scopesettings.channel1.min)+scopesettings.channel1.traceposition);//*settings->input_calibration[voltperdiv];;
    //scopesettings.channel1.triggerverticalposition = scopesettings.channel1.traceposition + (scopesettings.channel1.max-scopesettings.channel1.center)*2097152/ settings->input_calibration[voltperdiv];    
  }
  else
  {
    //Use the channel 2 center value
    //scopesettings.triggerlevel = scopesettings.channel2.center;
    //scopesettings.channel2.triggerverticalposition = scopesettings.channel2.traceposition;
      
    scopesettings.channel2.triggerverticalposition = scopesettings.channel2.center-128+scopesettings.channel2.traceposition + (ch2_max-ch2_min)/2;  
    //scopesettings.channel2.triggerverticalposition = scopesettings.channel2.center-128+scopesettings.channel2.traceposition;
  }
...
void scope_do_auto_setup(void)
{
  ...
  //for AC mode
  //uint32 dochannel1 = (scopesettings.channel1.enable)&&(scopesettings.channel1.coupling); //0 is DC mode
  //uint32 dochannel2 = (scopesettings.channel2.enable)&&(scopesettings.channel2.coupling); //1 is AC mode 
  
  uint32 AC_mode_CH1 = scopesettings.channel1.coupling;
  uint32 AC_mode_CH2 = scopesettings.channel2.coupling;
  //uint32 AC_mode = 0;
  */
  /*
  uint32  ch1_min=255-scopesettings.channel1.min;
  uint32  ch1_max=scopesettings.channel1.max;
  uint32  ch2_min=255-scopesettings.channel2.min;
  uint32  ch2_max=scopesettings.channel2.max;
  */
  /*
  //Set the channel to AC coupling; settings->coupling = 1;
  //is DC mode,set ACmod, go autoset in AC mode and later switch to autoset DC mode.
  if ((!DC_AC_CH1)&&dochannel1) {scopesettings.channel1.coupling = 1;fpga_set_channel_coupling(&scopesettings.channel1);}
  if ((!DC_AC_CH2)&&dochannel2) {scopesettings.channel2.coupling = 1;fpga_set_channel_coupling(&scopesettings.channel2);}
  
  //Wait 5ms for switch ac to dc mode
  timer0_delay(5);
  */
/*
  ...
  if(dochannel1)
  {
    ...
    scopesettings.channel1.displayvoltperdiv = 5;   //5  AC mode set to 3
    //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
    match_volt_per_div_settings(&scopesettings.channel1);
  }
  ...
  if(dochannel2)
  {
    ...
    //Use the most sensitive hardware setting and set it in the FPGA
    //Set sample 100mV/div ( 5->100mV, 4->200mV, 3->500mV, 2->1V, 1->2.5V, 0->5V )
    scopesettings.channel2.displayvoltperdiv = 5;   //5
    //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
    match_volt_per_div_settings(&scopesettings.channel2);
  }
  ...
  //for(settings->displayvoltperdiv=0;settings->displayvoltperdiv < 6;settings->displayvoltperdiv++)
   // while(!settings->frequencyvalid)
   // {
   ...
   //if(settings->displayvoltperdiv < 6)
     ...
     //fpga_read_sample_data(&scopesettings.channel1, 5);
  ...
  for(samplerate=0;samplerate<4;samplerate++)
  {
    ...
    //for AC ACoverload=0; for DCoverload=254
    if ((scopesettings.channel1.ADoverload > 254)&&(!AC_mode_CH1))      // hore je 254, dole je 1
    {
    ...
    //for AC ACoverload=0; for DCoverload=254
    if ((scopesettings.channel2.ADoverload > 254)&&(!AC_mode_CH2))      // hore je 254, dole je 1
    {
    ...
    //scopesettings.channel2.traceposition = 200;//100
       //  fpga_do_conversion();
         //fpga_read_sample_data(&scopesettings.channel1, 5);//100
  */
        /* 
  ...
  */
  ...
  /*  
    fpga_do_conversion();
    fpga_read_sample_data(&scopesettings.channel1, 5);//100
    while(settings->ADoverload) 
        {
            
        if ((scopesettings.channel1.ADoverload > 254))      // hore je 254, dole je 1
        ...
        if (settings->ADoverload == 254) 
            {settings->ADoverload = 0;
            if (settings->dcoffset<220) settings->dcoffset+=4; 
            else {settings->dcoffset=0; 
                  if(settings->displayvoltperdiv > 0) settings->displayvoltperdiv -= 1;
                  match_volt_per_div_settings(&scopesettings.channel1);
                  //timer0_delay(50);
                }
            }  
        if (settings->ADoverload == 1) 
            {settings->ADoverload = 0;
            if (settings->dcoffset>-200) settings->dcoffset-=4; 
            else {settings->dcoffset=0; 
                  if(settings->displayvoltperdiv > 0) settings->displayvoltperdiv -= 1;
                  match_volt_per_div_settings(&scopesettings.channel1);//fpga_set_channel_voltperdiv(settings);
                  //timer0_delay(50);
                }
            } 
        ...
        //Get the data from a sample run
        fpga_read_sample_data(settings, 5);
        }
    
    if ((settings->dcoffset)>0){settings->dcoffset+=8; fpga_set_channel_offset(settings);}
    if ((settings->dcoffset)<0){settings->dcoffset-=8; fpga_set_channel_offset(settings);}
    
    //Update the display
    scope_channel_settings(settings, 0);
    
    uint32  AD_min=255-settings->min;
    uint32  AD_max=settings->max;
    
    //Display correction for DC offset 
    if(AD_max>(AD_min+2)) {settings->traceposition-=((AD_max-AD_min)*2/settings->samplevoltperdiv)+settings->dcoffset/4;if (settings->traceposition>396)settings->traceposition=0;}//403
    if(AD_max<(AD_min-2)) {settings->traceposition+=((AD_min-AD_max)*2/settings->samplevoltperdiv)-settings->dcoffset/4;}
    
     //-**********************************************************************************
      display_set_fg_color(BLACK_COLOR);
      //Fill the settings background
      display_fill_rect(600, 0, 80, 50);  //x , y , sirka, vyska
      display_set_fg_color(WHITE_COLOR);
      display_set_font(&font_2);
      //display_text(650, 10, buffertime);
      display_decimal(600, 0, scopesettings.channel1.displayvoltperdiv);
      display_decimal(600, 10, scopesettings.channel1.max);
      display_decimal(600, 20, 255-scopesettings.channel1.min);
      
      display_decimal(600, 30, scopesettings.channel1.average);
      //display_decimal(600, 25, 255-scopesettings.channel1.min);
      
      display_decimal(630, 20, settings->ADoverload);
    */
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      /*
   //   while(1);
   ...
   */  
    ...
    scope_acqusition_settings(0);
      ...
      display_decimal(630, 10, scopesettings.timeperdiv);
    //while(1);    
    timer0_delay(3000);//9  
     */
    /*
...
/*
  ...
  if(dochannel1 && dochannel2 && (scopesettings.xymodedisplay == 0))
  {
    ...
    scopesettings.channel2.traceposition = 100;//100
 ...
 //----------------------------
  ...
  if(settings->frequencyvalid)
  {
    ...
    if(dochannel2)
    {
        fpga_read_sample_data(&scopesettings.channel2, 5);
      dochannel2 = scope_check_channel_range(&scopesettings.channel2);
    }
  }

  
        //display_decimal(600, 30, scopesettings.channel1.displayvoltperdiv);
        //timer0_delay(1000); //3
  ...
  if(dochannel1 || dochannel2)
  {
    ...
    if(dochannel1)
    {
      ...
      if(AC_mode_CH1)scopesettings.channel1.displayvoltperdiv = 3;
        else scopesettings.channel1.displayvoltperdiv--; 
      //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
      match_volt_per_div_settings(&scopesettings.channel1);
    }
    ...
    {
      //Use the 500mV/div setting and set it in the FPGA
      if(AC_mode_CH2)scopesettings.channel2.displayvoltperdiv = 3;  
      scopesettings.channel2.displayvoltperdiv--;
      //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
      match_volt_per_div_settings(&scopesettings.channel2);
    }
  
          //  display_decimal(630, 20, scopesettings.channel1.displayvoltperdiv);
        //timer0_delay(1000); //2
    ...
    //display_decimal(630, 30, scopesettings.channel1.displayvoltperdiv);
    //timer0_delay(1000);//3

    //If one or either channel not in range another conversion is needed
    if(dochannel1 || dochannel2)
    {
      //Check if channel 1 still needs to be done //scope_do_auto_setup
      if(dochannel1)
      {
        //Use the 2.5V/div setting and set it in the FPGA
        if(AC_mode_CH1)scopesettings.channel1.displayvoltperdiv = 1;
            else scopesettings.channel1.displayvoltperdiv--;
        //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
        match_volt_per_div_settings(&scopesettings.channel1);
      }
      ...
      {
        //Use the 2.5V/div setting and set it in the FPGA
        if(AC_mode_CH2)scopesettings.channel2.displayvoltperdiv = 1;
            else scopesettings.channel2.displayvoltperdiv--;
        //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
        match_volt_per_div_settings(&scopesettings.channel2);
      }
      
      //display_decimal(630, 40, scopesettings.channel1.displayvoltperdiv);
      //timer0_delay(1000);
      ...
      if(dochannel1 || dochannel2)
      {
        ...
        if(dochannel1)
        {
          ...
          if(AC_mode_CH1)scopesettings.channel1.displayvoltperdiv = 0;
            else scopesettings.channel1.displayvoltperdiv--;
        ...
        {
          //Set the lowest sensitivity for this channel
          if(AC_mode_CH2)scopesettings.channel2.displayvoltperdiv = 0;
            else scopesettings.channel2.displayvoltperdiv--;
 ...
 */
    /*
    //Display correction for DC offset CH1
    if(ch1_max>(ch1_min+2)) {scopesettings.channel1.traceposition-=(ch1_max-ch1_min);}
    if(ch1_max<(ch1_min-2)) {scopesettings.channel1.traceposition+=(ch1_min-ch1_max); }
      */
   //----------------------- 
     /*
    if(ch1_max>(ch1_min+2)) {scopesettings.channel1.traceposition-=(ch1_max-ch1_min); scopesettings.channel1.samplevoltperdiv += 1;}
    if(ch1_max<(ch1_min-2)) {scopesettings.channel1.traceposition+=(ch1_min-ch1_max); scopesettings.channel1.samplevoltperdiv += 1;}
     fpga_set_channel_voltperdiv(&scopesettings.channel1);
        */
    
    //if(ch1_max>ch1_min) {scopesettings.channel1.traceposition-=(ch1_max-ch1_min); scopesettings.channel1.dcoffsetCH1-=(ch1_max-ch1_min);} 
    //if(ch1_max<ch1_min) {scopesettings.channel1.traceposition+=(ch1_min-ch1_max); scopesettings.channel1.dcoffsetCH1+=(ch1_min-ch1_max);} 
        //else scopesettings.channel1.dcoffsetCH1=0;
  /*
    //Update the display
    scope_channel_settings(&scopesettings.channel1, 0);
  }
  ...
  if(scopesettings.channel2.enable)
  {
    ...
    //Display correction for DC offset CH2
    //if(ch2_max>(ch2_min+2)) scopesettings.channel2.traceposition-=(ch2_max-ch2_min);
    //if(ch2_max<(ch2_min+2)) scopesettings.channel2.traceposition+=(ch2_min-ch2_max);
  ...
  */
  /*
  dochannel1 = scopesettings.channel1.enable;
  dochannel2 = scopesettings.channel2.enable;
  
  //IF DC mode, switch to DC and go autoset sensitivity in DC mode
  //Set the channel to DC coupling;
  //settings->coupling = 0;
  if ((!DC_AC_CH1)&&dochannel1) {scopesettings.channel1.coupling = 0;fpga_set_channel_coupling(&scopesettings.channel1);scope_autoset_DC(&scopesettings.channel1);}
  if ((!DC_AC_CH2)&&dochannel2) {scopesettings.channel2.coupling = 0;fpga_set_channel_coupling(&scopesettings.channel2);scope_autoset_DC(&scopesettings.channel2);}
  
  //&scopesettings.channel1
  */
  // go to autosetup DC mode
  //scope_do_auto_DC_setup();
...
void scope_autoset_DC(PCHANNELSETTINGS settings)
{
    //Wait 5ms for switch ac to dc mode
    timer0_delay(5);
    ...
    while(settings->ADoverload) 
        ...
        if (settings->ADoverload == 255) 
            ...
            if (settings->dcoffset<220) settings->dcoffset+=4; 
                  ...
                  if(settings->samplevoltperdiv > 0) settings->samplevoltperdiv -= 1;
    ...
    /*
      display_set_fg_color(BLACK_COLOR);
      //Fill the settings background
      display_fill_rect(600, 5, 50, 30);  //x , y , sirka, vyska
      display_set_fg_color(WHITE_COLOR);
      display_set_font(&font_2);
      //display_text(650, 10, buffertime);
      display_decimal(600, 5, settings->dcoffset);
      //display_decimal(600, 15, 255-scopesettings.channel1.min);
    ...
    */
    ...
    /*
      ...
      display_set_font(&font_2);
      //display_text(650, 10, buffertime);
      display_decimal(600, 0, ch1_max);
      display_decimal(600, 10, ch1_ch1_minmin);
      display_decimal(600, 20, ch1_max-ch1_min);
      display_decimal(600, 30, settings->samplevoltperdiv);
    */
    ...
    //if(ch1_max>(ch1_min+2)) {scopesettings.channel1.traceposition-=(ch1_max-ch1_min)/settings->samplevoltperdiv;}
    //if(ch1_max<(ch1_min-2)) {scopesettings.channel1.traceposition+=(ch1_min-ch1_max)/settings->samplevoltperdiv;}
    //if(ch1_max>(ch1_min+2)) {scopesettings.channel1.traceposition-=(ch1_max-ch1_min)/settings->input_calibration[voltperdiv];}
...
void scope_display_trace_data(void)
{
 ...
 // uint32    center; 
    ...
    */
      ...
      //settings->input_calibration
      
      //display_decimal(600, 0,  scopesettings.channel1.adc1compensation);
      //display_decimal(600, 10, scopesettings.channel1.adc2compensation);
      //display_decimal(600, 20, scopesettings.channel2.adc1compensation);
      //display_decimal(600, 30, scopesettings.channel2.adc2compensation);
  ...
  display_set_fg_color(BLACK_COLOR);
  display_fill_rect(2, 44, 728, 434);
...
void scope_display_cursor_measurements(void)
{
  ...
  if(scopesettings.timecursorsenable || (scopesettings.voltcursorsenable && (scopesettings.channel1.enable || scopesettings.channel2.enable)))
  {
    ...
    display_fill_rounded_rect(25, 47, 88, height, 2);//5
    ...
    if(scopesettings.voltcursorsenable)
    {
      ...
      if(scopesettings.channel1.enable)
      {
        ...
        Y_1 = 447 - scopesettings.voltcursor1position - scopesettings.channel1.traceposition;
        //Get the volts Y_2 based on the cursor position
        Y_2 = 447 - scopesettings.voltcursor2position - scopesettings.channel1.traceposition;
      ...
      if(scopesettings.channel2.enable)
      {
        ...
        //Get the volts Y_1 based on the cursor position
        Y_1 = 447 - scopesettings.voltcursor1position - scopesettings.channel2.traceposition;
        //Get the volts Y_2 based on the cursor position
        Y_2 = 447 - scopesettings.voltcursor2position - scopesettings.channel2.traceposition;
...
{
  //Check if channel1 is active and hide measurements is 0-off
  if((scopesettings.channel1.enable)&&(!scopesettings.hide_values_CH1))
  {
  ...
  //Check if channel2 is active and hide measurements is 0-off
  if((scopesettings.channel2.enable)&&(!scopesettings.hide_values_CH2))
  {
    //Display the enabled measurements if it is active
...
void scope_reset_config_data(void)
{
  ...
  scopesettings.screenbrightness = 50;
...
void scope_save_config_data(void)
{
  ...
  *ptr++ = scopesettings.channel2.adc2compensation;
  ...
  //Point to the calibration settings
  ptr32 = (uint32*)&settingsworkbuffer[CALIBRATION_SETTING_OFFSET + 25];
    
  //If inputs_calibration DONE, Copy the working set values to the saved values
  //if ( calibrationopen == 9)
  //{  
