...
void fpga_set_channel_offset(PCHANNELSETTINGS settings)
{
  ...
  //fpga_write_short((settings->dc_calibration_offset[settings->samplevoltperdiv])+202+scopesettings.channel1.dcoffsetCH1);
  //fpga_write_short(settings->dc_calibration_offset[settings->samplevoltperdiv]+scopesettings.channel1.dcoffset);
  ...
  //fpga_write_short(settings->dc_calibration_offset[settings->samplevoltperdiv]+202-(((int32)scopesettings.channel1.traceposition * 8294304) / signal_adjusters[voltperdiv]));

  //fpga_write_short(settings->dc_calibration_offset[settings->samplevoltperdiv]+440-(((int32)scopesettings.channel1.traceposition * 8294304) / signal_adjusters[voltperdiv]));
}
...
uint16 fpga_prepare_for_transfer(void)
{
  ...
  data = (fpga_read_short() & 0x0FFF);//zmena 11.12.2023
