...
void scope_do_auto_setup(void)
{
 ...
 // uint32    center; 
  ...
  //max = (scopesettings.channel1.max * settings->input_calibration[settings->samplevoltperdiv]) >> VOLTAGE_SHIFTER;
/*
  //Scale the sample based on the two volt per div settings when they differ
  if(scopesettings.channel1.displayvoltperdiv != scopesettings.channel1.samplevoltperdiv)
  {
    //Scaling factor is based on the two volts per division settings
    max = (scopesettings.channel1.max * vertical_scaling_factors[scopesettings.channel1.displayvoltperdiv][scopesettings.channel1.samplevoltperdiv]) / 10000;
  }
  */
    max1 = (scopesettings.channel1.max * signal_adjusters[scopesettings.channel1.displayvoltperdiv]) >> VOLTAGE_SHIFTER;
    min1 = (scopesettings.channel1.min * signal_adjusters[scopesettings.channel1.displayvoltperdiv]) >> VOLTAGE_SHIFTER;
    p2p1 = (scopesettings.channel1.peakpeak * signal_adjusters[scopesettings.channel1.displayvoltperdiv]) >> VOLTAGE_SHIFTER;
    
    //Check the channel data if not yet done
    if(flags & 0x01)
    {
      //Check if signal in good range 
      if((max1 >= 301) || (min1 < 100) || (p2p1 > minvpplevel))
    ...
    {
      //Check if signal in good range 
      if((scopesettings.channel2.max >= 301) || (scopesettings.channel2.min < 100) || (scopesettings.channel2.peakpeak > minvpplevel))  
   ...
   center1x = (scopesettings.channel1.center * signal_adjusters[scopesettings.channel1.displayvoltperdiv]) >> VOLTAGE_SHIFTER;
   //center1x = (41956 * scopesettings.channel1.center  * signal_adjusters[scopesettings.channel1.displayvoltperdiv]) >> 22;
  ...
  if(scopesettings.channel1.enable)
  {
    ...
    if(center1x < center1)
    {
      ...
      scopesettings.channel1.traceposition += (center1 - center1x);
  ...
  if(scopesettings.channel2.enable)
  {
    ...
    //Adjust the center line based on targeted center
    if(scopesettings.channel2.center < center2)
    {
      //Calculate a new trace offset
        scopesettings.channel2.traceposition += (center2 - scopesettings.channel2.center);
    ...
    {
      //Calculate the needed offset
      offset = scopesettings.channel2.center - center2;
...
{ 
  ...
  if(mode == 0)
  {
    //Check if run or stop mode - Run mode:Green box 
  if(scopesettings.runstate == 0) {display_set_fg_color(GREEN_COLOR);}
        //Stop mode:Red box 
...
void scope_run_stop_text(void)
{
  ...
  {
    //Run mode. Green box
    if(scopesettings.triggermode==1) display_set_fg_color(ORANGE_COLOR); 
  ...
  if(scopesettings.runstate == 0)
  {
    //Run mode. Black text 
    display_set_fg_color(BLACK_COLOR);
    if(scopesettings.triggermode==1) display_text(RUN_STOP_TEXT_XPOS + 3, RUN_STOP_TEXT_YPOS + 1, "WAIT");
...
//***********************************************************************
void scope_do_auto_DC_setup(void)
{
  ...
  //uint32 DC_AC_CH1 = scopesettings.channel1.coupling;
  //uint32 DC_AC_CH2 = scopesettings.channel2.coupling;
  /*
  ...
  */
  ...
  if(dochannel1)
  {
    ...
    scopesettings.channel1.displayvoltperdiv = 5;
  ...
  if(dochannel2)
  {
    ...
    scopesettings.channel2.displayvoltperdiv = 5;
    //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
  ...
  if(dochannel1 || dochannel2)
  {
    ...
    if(dochannel1)
    {
      ...
      scopesettings.channel1.displayvoltperdiv = 3;
      //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
      match_volt_per_div_settings(&scopesettings.channel1);
    }
    ...
    {
      //Use the 500mV/div setting and set it in the FPGA
      scopesettings.channel2.displayvoltperdiv = 3;
      //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
      match_volt_per_div_settings(&scopesettings.channel2);
    }
    ...
    if(dochannel1 || dochannel2)
    {
      ...
      if(dochannel1)
      {
        ...
        scopesettings.channel1.displayvoltperdiv = 1;
        //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
        match_volt_per_div_settings(&scopesettings.channel1);
      }
      ...
      {
        //Use the 2.5V/div setting and set it in the FPGA
        scopesettings.channel2.displayvoltperdiv = 1;
        //Set fpga samplevoltperdiv=displayvoltperdiv; Set fpga dcoffset
        match_volt_per_div_settings(&scopesettings.channel2);
      }
  ...
  if(scopesettings.channel2.enable)
  {
   ...
   /*
      ...
      display_decimal(600, 5, scopesettings.channel1.max);
...
void scope_system_settings_hour_item(void)
{
  ...
  display_set_fg_color(WHITE_COLOR);
  display_set_bg_color(0x00404040);
...
void scope_system_settings_ONOFF_RTC_item(void)
{
  ...
  //Show the state
  scope_display_slide_button(530, 419, scopesettings.onoffRTC);
}
...
void scope_display_slide_button(uint16 xpos, uint16 ypos, uint8 state)
{
  ...
  uint32 edgecolor  = LIGHTGREY1_COLOR;
...
void scope_draw_grid(void)
{
  ...
  if (kurzor_position > 686) kurzor_position = 686; //600 ma byt
    //Draw the edge
  display_draw_rect(kurzor_position, 454, 40, 14);//5
  ...
  if(scopesettings.gridbrightness > 3)
  {
    ...
    //Draw the edge
    display_draw_rect(2, 44, 726, 406);//46 404

    //Draw the center lines
    display_draw_horz_line(249,  2, 726);
    display_draw_vert_line(364, 44, 448);
